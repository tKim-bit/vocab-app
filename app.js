...
if (words.length === 0) {
  fetch("words.json")
    .then(r => r.json())
    .then(d => { 
        // 初期単語リストがない場合、words.jsonから読み込み
        words = d; 
        save(); 
        renderLearn(); 
    })
    .catch(error => {
        // words.jsonの読み込みに失敗した場合のフォールバック処理
        console.error("Failed to load initial words from words.json:", error);
        // デフォルトの単語リストを設定（もしあれば）
        words = [
            { en: "apple", ja: "りんご" },
            { en: "banana", ja: "バナナ" },
            { en: "cat", ja: "猫" }
...
function renderList() {
  ...
  // チェックボックス付きのリストを生成
  // ulとボタンは#viewの直接の子要素として配置されているため、表示されるはずです。
  view.innerHTML = `
    <ul id="wordList" class="space-y-3 list-none p-0">
    ${words.map((w, index) => `
        <!-- liにflexを適用し、チェックボックスとテキストを中央揃えで配置 -->
        <li class="flex items-center border-b pb-2 last:border-b-0">
            <!-- チェックボックスを追加し、data-indexで配列のインデックスを保持 -->
            <!-- 補足: words配列の要素（w）自体は削除時にインデックスが変わるため、
                 ここではmapのループインデックス（index）を使用し、後で処理します -->
    ...
    <!-- 削除ボタンを追加: #viewがflex-columnなので、リストの下に表示される -->
  ...
  $("#deleteSelected").onclick = () => {
    ...
    // 重要なポイント: この index は renderListが呼ばれた時点のwords配列のインデックスです。
    // そのため、必ず降順で処理する必要があります。
    ...
    indicesToDelete.forEach(index => {
      // spliceを使って元の配列から要素を削除
      words.splice(index, 1);
    });
    
    save(); // localStorageを更新
    // 連続正解数をリセット（単語リストが変わったため）
